...
{ oc get project ${OPENSHIFT_PRIMARY_PROJECT} 2>&1 >/dev/null && oc project ${OPENSHIFT_PRIMARY_PROJECT}; } || oc new-project ${OPENSHIFT_PRIMARY_PROJECT} ${OPENSHIFT_PROJECT_DESCRIPTION:+"--description"} ${OPENSHIFT_PROJECT_DESCRIPTION} ${OPENSHIFT_PROJECT_DISPLAY_NAME:+"--display-name"} ${OPENSHIFT_PROJECT_DISPLAY_NAME} || { echo "FAILED: Could not use indicated project" && exit 1; }
echo "Done."
